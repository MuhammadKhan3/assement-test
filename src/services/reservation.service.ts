 ...
 */
export async function reserveItems(userId: string, items: ReservationItem[]) {
    // Validate all products exist
    const productIds = items.map((i) => i.productId);
    const products = await productRepository.getAllByIds(productIds);
    ...
    await redisManager.ensureStockKeys(products);
    ...
    const result = await redisManager.reserveBatch(userId, batchItems);
    ...
    await reservationRepository.createMany(
        items.map((item) => ({
            userId,
            productId: item.productId,
            quantity: item.quantity,
 ...
 */
export async function cancelReservation(userId: string, productId: string) {
    const product = await productRepository.getUniqueById(productId);
    if (!product) throw new NotFoundError("Product");

    const restoredQty = await redisManager.release(userId, productId);

    if (restoredQty === 0) {
        throw new NotFoundError("Active reservation for this product");
    }

    await reservationRepository.markStatus(userId, productId, ReservationStatus.CANCELLED);
 ...
 */
export async function checkout(userId: string, items: ReservationItem[]) {
    const completedItems: { productId: string; quantity: number; totalPrice: number }[] = [];

    await prisma.$transaction(async (tx) => {
        for (const item of items) {
            const reservedQty = await redisManager.checkout(userId, item.productId);
            ...
            if (reservedQty !== item.quantity) {
                await redisManager.incrStock(item.productId, reservedQty);
                throw new BadRequestError(
 ...
 */
export async function handleReservationExpiry(userId: string, productId: string) {
    console.log(`🕐 Reservation expired: user=${userId} product=${productId}`);

    const reservation = await reservationRepository.findActive(userId, productId);

    if (reservation) {
        await redisManager.incrStock(productId, reservation.quantity);
        console.log(`♻️  Restored ${reservation.quantity} units to product ${productId}`);
    }

    await reservationRepository.markStatus(userId, productId, ReservationStatus.EXPIRED);
}
